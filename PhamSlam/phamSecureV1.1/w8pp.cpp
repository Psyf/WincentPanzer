...
// *********************** //
// Pham
typedef std::tuple<float,float,_u8> data_node_T;
...
void updateBetterMap(){
    ...
    if (allow_scanning == 0)
        return;
    if ((has_been_scanned==0) || ((current_cmd!='l')&&(current_cmd!='r'))) {
        ...
        while(fscanf(betterMapFile,"%d %d %d",&preX,&preY,&brightness)>=0){
            ...
            newY = preY*cos(rad) - preX*sin(rad);
...
u_result capture_and_display(RPlidarDriver * drv, const char* fname)
{
    ...
    switch(current_cmd) {
        case 'l':
            command_angle = (-1)*current_cmd_val;
            previous_min_angle += current_cmd_val;
            if (previous_min_angle > 180)
                previous_min_angle -= 360;
        case 'r':
    ...
    if (IS_OK(ans) || ans == RESULT_OPERATION_TIMEOUT) {
        ...
        min_angle = 0;
        min_angle_set.clear();
        float angle_scanned_0 = 182;
        float angle_scanned_90 = 272;
        float angle_scanned_180 = 2;
        float angle_scanned_270 = 92;;
        // **************************************** //
        ...
        for (int pos = 0; pos < (int)count ; ++pos) {
            ...
            if ((min_dist==0)||(dist<min_dist)) {
                ...
                min_angle = angle;
                min_angle_set.clear();
                min_angle_diff = std::min(fabs(min_angle-previous_min_angle),360-fabs(min_angle-previous_min_angle));
            }
            if (dist==min_dist) {
                min_angle_set.push_back(angle);
                float local_angle_diff = std::min(fabs(angle-previous_min_angle),360-fabs(angle-previous_min_angle));
            ...
            data_list.push_back({endptX,endptY,quality});
            data_list_size += 1;
        ...
        if (min_angle > 180)
            min_angle -= 360;
        min_angle_diff = min_angle - previous_min_angle;
        if (min_angle_diff <= -180)
            min_angle_diff += 360;
        if (min_angle_diff > 180)
            min_angle_diff -= 360;
        delta_angle = command_angle - min_angle_diff;
        if (delta_angle <= -180)
            delta_angle += 360;
        if (delta_angle > 180)
            delta_angle -= 360;
