...
float delta_angle = 0;
char current_cmd = '0';
float current_cmd_val = 0;

void clear_map_variables() {
    current_X = 0;
    current_Y = 0;
    current_angle = 0;
    delta_dist = 0;
...
void updatePosition(char cmd, uint32_t dist){
    current_cmd = cmd;
    current_cmd_val = dist;
    switch(cmd){
        case 'f':
        case 'F':
            current_X += (float) (dist*sin(current_angle*PI/180.0));
            current_Y += (float) (dist*cos(current_angle*PI/180.0));
            break;
        case 'b':
        case 'B':
            current_X -= (float) (dist*sin(current_angle*PI/180.0));
...
void startServer(int portNum)
{
	...
	// Create the context
	//NOT SSL_CTX ctx because global? 
	ctx = create_context(); 
