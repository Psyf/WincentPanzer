...
void reverse(float dist, float speed)
{
  ...
  // RF = Right forward pin, RR = Right reverse pin
  // This will be replaced later with bare-metal code.
  analogWrite(LR, val-outputRight);
  analogWrite(RR, val-outputLeft);
  analogWrite(LF, 0);
  analogWrite(RF, 0);
}
...
void setup() {
  ...
  myLeftPID.SetSampleTime(5);
  myRightPID.SetSampleTime(5);  
  ...
  {  inCommand = 1; 
      if (dir == FORWARD)
      {
          ...
          if (forwardDist < newDist)
          {
            ...
            inputRight = rightForwardTicks-leftForwardTicks; 
            myLeftPID.Compute(); 
            myRightPID.Compute();
      ...
      else 
      {
          if (dir == BACKWARD)
          {
                inputLeft = leftBackwardTicks-rightBackwardTicks; 
                inputRight = rightBackwardTicks-leftForwardTicks; 
                myLeftPID.Compute(); 
                myRightPID.Compute();

              if (backwardDist < newDist) 
                  reverse((int)(newDist-backwardDist), 100); 
              else if (backwardDist >= newDist)
              {
                deltaDist = 0; 
                newDist = 0; 
                stop();
