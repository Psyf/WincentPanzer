...
volatile char isGap; //IR checks if the bot is at a junction
...
// Functions to be called by INT0 and INT1 ISRs (wheels).
ISR (INT0_vect)
{
    if (dir == FORWARD) 
    {
...
{
    unsigned long ticks = (unsigned long) ((ang/360.0)*(vincentCirc/WHEEL_CIRC)*COUNTS_PER_LREV/2); //last /2 by Saif. 
    //unsigned long ticks = (unsigned long) ((ang*PI/180.0)*3.85)/WHEEL_CIRC*COUNTS_PER_LREV;   //other proposed formula
    return ticks; 
}
...
// Ensures the bot goes in a straight line, using encoder ticks
void correctMotors() {
    if (dir == FORWARD)
    {
...
void loop() {
    ...
    //NOTE: We put counterCleanup in stop(). Comment it out for debugging.  
    if (deltaDist > 0)
    {  
        inCommand = 1; 
        if (dir == FORWARD)
    ...
    // we pop off the stack and check whether we are approaching a junction
    // by looking at the next command. 
    ...
    //check for gap if supposed to be at Junction while backtracking. 
    //if at junction and found a gap, ensure turn is made with enough room for error
    //if at junction and can't see gap, continue moving until gap is detected
    ...
    //if too close to a wall, do not execute turn 
    //move until it is safe to turn 
